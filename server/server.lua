...
local cooldowns = {}
Core.Callback.Register('bcc-waves:CheckCooldown', function(source, cb, location)
    local src = source
...
RegisterServerEvent('bcc-waves:RewardPayout', function(lootCfg)
    local src = source
    local user = Core.getUser(src)
    ...
    Core.NotifyRightTip(source,
        _U('youTook') .. '$~o~' .. cash .. '~q~, ~o~' .. gold .. '~q~ ' .. _U('gold') .. ', ~o~' .. rol .. '~q~ ' .. _U('rol'), 5000)
...
Core.Callback.Register('bcc-waves:RobberyCheck', function(source, cb)
    local src = source
    local user = Core.getUser(src)
    if not user then return cb(false) end
    local character = user.getUsedCharacter
    ...
    -- No police requirement for starting robbery
    cb(true)
end)

RegisterServerEvent('bcc-waves:DeletePed', function(netId)

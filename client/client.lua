...
RegisterCommand(Config.RobberyCommand, function()
    if not RobberyEnabled then
        local result = Core.Callback.TriggerAwait('bcc-waves:RobberyCheck')
        if result then
            RobberyEnabled = true
        ...
        for i, coords in ipairs(Markers) do
            ...
            if distance <= locationCfg.Distance then
                ...
                if IsControlPressed(0, 0xCEFD9220) then -- Hold E to start robbery
                    local result = Core.Callback.TriggerAwait('bcc-waves:CheckCooldown', locationCfg)
                    if result then
                        Inmission = true
                        TriggerEvent('bcc-waves:RobberyHandler', locationCfg)
                    else
...
AddEventHandler('bcc-waves:RobberyHandler', function(locationCfg)
    RequestStreamedTextureDict("menu_textures")
    while not HasStreamedTextureDictLoaded("menu_textures") do
        Wait(0)
    end
    ...
    if locationCfg.EnemyNpcs then
        TriggerEvent('bcc-waves:EnemyPeds', locationCfg)
    end
    ...
    Countdown = true
    TriggerEvent('bcc-waves:Countdown', locationCfg)
    ...
    for _, lootCfg in pairs(locationCfg.LootLocations) do
        TriggerEvent('bcc-waves:LootHandler', lootCfg, startingCoords)
    end
end)

AddEventHandler('bcc-waves:Countdown', function(locationCfg)
...
AddEventHandler('bcc-waves:LootHandler', function(lootCfg, startingCoords)
    ...
    while true do
        ...
        if distance < 2 then
            ...
            if lootPrompt:HasCompleted() then
                ...
                TriggerServerEvent('bcc-waves:RewardPayout', lootCfg)
                Inmission = false
                break
            end
        end
...
AddEventHandler('bcc-waves:EnemyPeds', function(location)
        ...
        if DoReset or LootDone then
            ...
            for _, ped in pairs(enemyPeds) do
                if DoesEntityExist(ped) then
                    ...
                    TriggerServerEvent('bcc-waves:DeletePed', netId)
                end
            end
            -- Remove blips
